...
def login(creds):
    ...
    response = requests.request("POST", url, headers=headers, data = payload, verify=False, timeout=5).json()
    return response
...
def create_vrf(token):
    ...
    response = requests.request("POST", url, headers=headers, data=json.dumps(payload), verify=False, timeout=5)
    print(response)
