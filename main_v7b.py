...
def url_ok(uri, head, pay, request_method):
    ...
    try:
        ...
        response = requests.request(request_method, uri, headers=head, data=pay, verify=False)
...
def check_vrf_network_existance(vrf_net, token):
    ...
    resp = requests.request("GET", vrf_net, headers=headers, data=payload, verify=False)
...
def checking_state(token):
    ...
    while count > 0:
        if count == 1:
            ...
            count -= 1
            print(state)
            print(f"-> PASS A:{count}: Fabric not in steady state")
            recal_save(token)
            state = get_deployment_state(token)
        elif state == "PENDING":
            recal_save(token)
            ...
            print("-> Success: Network has been deployed.")
            count = 0
